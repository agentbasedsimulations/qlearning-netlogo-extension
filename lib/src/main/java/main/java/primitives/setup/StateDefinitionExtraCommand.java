...
public class StateDefinitionExtraCommand implements org.nlogo.api.Command {
    ...
    public Syntax getSyntax() {
        return SyntaxJ.commandSyntax(new int[] {Syntax.ListType(), Syntax.ReporterType()});
    }

    @Override
    public void perform(Argument[] args, Context context) throws ExtensionException {
        AgentLearning agentLearning =  Session.getInstance().getAgent(context.getAgent());
        ...
        if(agentLearning == null) {
            ...
            stateDef.setReporterAux(args[1].getReporter());
